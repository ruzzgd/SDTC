...
<template>
<div class="relative h-screen w-ful flex flex-col">
    <nav class="w-full h-[130px] flex flex-col border-b bg-gray-100 border-gray-200 shadow-[0_2px_5px_rgba(135,206,235,0.3)] z-30">
        <div class="flex flex-row justify-between items-center py-3 px-6">
            <div>
                <img src="@/assets/img/sdtc-logo.jpg" alt="logo" class="h-[50px] bg-gray-900 rounded-2xl" />
            </div>

            <!-- Profile icon with dropdown -->
            <div class="relative">
                <button @click="toggleProfileDropdown" class="hover:text-gray-600 transition">
                    <i class="fa-solid fa-circle-user text-3xl text-gray-900 cursor-pointer"></i>
                </button>

                <div v-if="profileDropdown" class="absolute right-0 mt-2 w-56 bg-gray-200 shadow-lg rounded-lg overflow-hidden z-50">
                    <div class="flex items-center gap-2 px-4 py-2 text-gray-900 text-sm truncate">
                        <i class="fa-solid fa-envelope text-blue-400"></i>
                        <span>{{ adminEmail }}</span>
                    </div>
                    <button @click="showChangePassModal = true; profileDropdown = false"
                        class="w-full text-left px-4 py-2 hover:bg-gray-100 text-sky-500 flex items-center gap-2">
                        <i class="fa-solid fa-key text-green-400"></i> Change Password
                    </button>
                    <button @click="logout" class="w-full text-left px-4 py-2 hover:bg-gray-100 text-red-600 flex items-center gap-2">
        ...
        <div class="w-full h-[50px] flex items-center gap-5 shadow-sm px-6">
            <button class="px-4 py-1.5 text-sm font-medium rounded transition" :class="isActive('/admin/dashboard')
                ...
                : 'bg-sky-500 text-gray-700 hover:bg-sky-600'"
            ...
            <div class="relative">
                ...
                <div v-if="orderDropdown" class="absolute mt-1 left-0 bg-gray-200 shadow rounded w-48 py-2 z-50">
                    <button class="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100" :class="isActive('/admin/order-management/approvals')
                        ? 'font-bold text-gray-700'
                        : 'text-gray-900'"
                        @click="goTo('/admin/order-management/approvals')">
                        <i class="fa-solid fa-circle-check text-lg"></i> Approvals
                    </button>
                    <button class="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100" :class="isActive('/admin/order-management/logs')
                        ? 'font-bold text-gray-700'
                        : 'text-gray-900'"
    ...
    <div v-if="showChangePassModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
        <div class="bg-gray-200 p-6 rounded-lg w-96 text-gray-900">
            ...
            <div v-if="!codeVerified">
                <div class="mb-3 flex items-center gap-2">
                    <i class="fa-solid fa-envelope text-blue-400"></i>
                    <input type="email" v-model="adminEmail" class="bg-gray-100 text-gray-900 border border-gray-200 rounded px-3 py-2 w-full" readonly />
                </div>
                <div class="mb-3 flex items-center gap-2">
                    <i class="fa-solid fa-shield-halved text-yellow-400"></i>
                    <input type="text" v-model="verificationCode" placeholder="Enter verification code" class="bg-gray-100 text-gray-900 border border-gray-200 rounded px-3 py-2 w-full" />
                </div>
                <div class="flex justify-end gap-2">
                    <button @click="sendVerification" class="bg-blue-600 px-3 py-1 rounded hover:bg-blue-700 text-gray-900">Send Code <span v-if="sendCooldown>0">({{ sendCooldown }}s)</span></button>
                    <button @click="verifyCode" class="bg-green-600 px-3 py-1 rounded hover:bg-green-700 text-gray-900">Verify</button>
                </div>
            </div>
            ...
            <div v-else>
                <div class="mb-3 flex items-center gap-2">
                    ...
                    <input type="password" v-model="newPassword" placeholder="New Password" class="bg-gray-700 text-gray-900 border border-gray-100 rounded px-3 py-2 w-full" />
                </div>
                <div class="mb-3 flex items-center gap-2">
                    <i class="fa-solid fa-lock text-blue-400"></i>
                    <input type="password" v-model="confirmPassword" placeholder="Re-enter Password" class="bg-gray-100 text-gray-900 border border-gray-600 rounded px-3 py-2 w-full" />
                </div>
            </div>

            <div class="flex justify-end gap-2 mt-4">
                <button @click="showChangePassModal=false; codeVerified=false" class="bg-gray-400 px-4 py-2 rounded hover:bg-gray-600">Cancel</button>
                <button v-if="codeVerified" @click="changePassword" class="bg-blue-600 px-4 py-2 rounded hover:bg-blue-700">Submit</button>
            </div>
        </div>
    </div>

    <div class="flex-1 overflow-auto p-6 bg-gray-300 text-gray-900">
        <router-view />
    </div>
</div>
</template>
