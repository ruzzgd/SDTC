...
<template>
    <div v-if="orderStore.showModal" class="absolute inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
        <div
            class="bg-gray-200 w-full max-w-4xl rounded-2xl shadow-lg overflow-hidden flex flex-col md:flex-row h-full sm:max-h-[400px] relative max-h-[540px]">

            <!-- Left: Image -->
            <div class="md:w-1/2 flex items-center justify-center bg-gray-200 p-4">
            ...
            <div class="md:w-1/2 p-6 flex flex-col justify-between overflow-y-scroll custom-scrollbar">
                <div class="space-y-2">
                    ...
                    <p class="text-gray-700 flex items-center gap-2">
                        <i class="fa-solid fa-boxes-stacked text-gray-800"></i>
                ...
                <div class="mt-4 flex flex-col gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        <button @click="decreaseQuantity"
                            class="bg-gray-400 hover:bg-gray-500 text-gray-900 w-10 h-10 flex items-center justify-center rounded-lg text-xl font-bold">
                            -
                        </button>
                        <input type="number" v-model.number="quantity" :max="maxStock" :min="1" @input="() => {
                            if (quantity > maxStock) quantity = maxStock;
                            if (quantity < 1) quantity = 1;
                        }" class="w-16 text-center rounded-lg bg-gray-400 text-gray-900 outline-none text-lg py-1" />
                        <button @click="increaseQuantity"
                            class="bg-gray-400 hover:bg-gray-500 text-gray-900 w-10 h-10 flex items-center justify-center rounded-lg text-xl font-bold">
                            +
                        </button>
                        <span class="text-gray-700 ml-2">Max: {{ maxStock }}</span>
                    </div>

                    <!-- Buttons -->
                    <div class="flex gap-3">
                        <button @click="addToCart"
                            class="flex-1 bg-sky-600 hover:bg-sky-700 text-gray-900 font-semibold py-3 rounded-xl transition hover:scale-105 flex items-center justify-center gap-2">
                            <i class="fa-solid fa-cart-plus"></i> Add to Cart
                        </button>
                        <button @click="placeOrder"
                            class="flex-1 bg-green-600 hover:bg-green-700 text-gray-900 font-semibold py-3 rounded-xl transition hover:scale-105 flex items-center justify-center gap-2">
            ...
            <button @click="orderStore.closeModal()"
                class="absolute top-0 right-0 text-gray-700 hover:text-gray-900 text-2xl m-5">
